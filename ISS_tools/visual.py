...
from ISS_tools.helpers import import_install, fit_line
...
def read_tsv(tsvfile):
    ...
    df=cleaning(df)
    return(DALIFrame(df))
    #>> DALIFrame.load_tsv(tsvfile) returns DALIframe

def read_dali_tsv(tsvfile):
    "reads header-less TSV file assumed to be output of ISS_rundali.py "
    dali_tsv_colnames=['cd1','sbjct','z-score','rmsd','ali-length','sbjct-length-cropped','seq-identity','qstarts','sstarts','lengths','rotation','translation','null1','null2']
...
def add_TRUE(df, CLAN, TRUE_df):
        ...
        df['TRUE'] = df['sbjct'].apply(lambda x: CLAN if x in match_sbjcts else 'unassigned')
...
class DALIFrame:
    ...
    def add_TRUE(self, CLAN, TRUE_df):
        ...
        # Initialize TRUE column if it doesn't exist
        self._df['TRUE'] = self._df['sbjct'].apply(lambda x: CLAN if x in match_sbjcts else 'unassigned')
    ...
    def grouplabel(self,k=10,labelin='clan',labelout='hue'):
    	...
    	unique_clans = self._df.sort_values('z-score', ascending=False)[labelin].dropna().drop_duplicates()
    ...
    def _standardize_columns(self):
        ...
        if 'accession' in self._df.columns and 'description' in self._df.columns:
            self._df['hovertext'] = self._df['sbjct'] + '|' + self._df['accession'] + ' ' + self._df['description']
        else:
    ...
    def set_motif(self, positions,k=4,mute=False):
        ...
        """
        # create motif column 
        self._df['motif'] = self._df['sequ-pileup'].apply(lambda s: ''.join([s[i] if len(s) > i else '-' for i in positions]))
        if not mute:
            print('\nFrequent')
        ...
        self._df['motif'] = self._df['motif'].where(self._df['motif'].isin(topk), other='other')
        if not mute:
            print('\nSelected')
    ...
    def scatter(self,x='z-score',y='ali-length',color=None, marker=None,
                left=None, right=None, bottom=None, top=None, title='Scatter Plot',
               hovertext=None,size=None):
        """interface to matplotlib scatterplots with continuous or categorical color,
        categorical markers, colorbar and marker legends
        """
        _scatter(self._df,x=x,y=y,color=color,marker=marker,
                left=left,right=right,bottom=bottom,top=top,title=title,size=size)

    def iscatter(self,x='z-score',y='ali-length',color='seq-identity',
                marker=None, left=None, right=None, bottom=None, top=None,
                hovertext='hovertext', title=None,size=None):
        ...
        if marker is not None:
            ...
            marker_is_cat=_is_categorical(series)
        if hovertext is not None:
            if not hovertext in self._df.columns: hovertext=None

        if not color_is_cat and marker is None: 
            return _scatter_plotly(self._df,x=x,y=y,color=color,hovertext=hovertext,title=title,size=size)
        if not color_is_cat and marker:
            return _scatter_plotly(self._df,x=x,y=y,color=color,marker=marker,hovertext=hovertext,title=title,size=size)
        if color_is_cat and marker is None:
            return _dual_marker_plotly(self._df,x=x,y=y,color=color,marker=color,hovertext=hovertext,title=title,size=size)
        if color_is_cat and marker:
            return _dual_marker_plotly(self._df,x=x,y=y,color=color,marker=marker,hovertext=hovertext,title=title,size=size)
        return
...
def _scatter(df, x='z-score', y='ali-length', color=None, marker=None, 
            left=None,right=None,bottom=None,top=None,title='Smart Scatter Plot',size=None):
        """
    ...
    - This function simplifies creating informative scatter plots directly from DataFrame columns.
        ...
        base_marker_styles = ['o', 's', '^', 'D', 'v', 'P', '*', 'X', '<', '>', 'H', 'h', 'd']
        ...
        if color is None:
            if marker is None:
                ax.scatter(df[x], df[y], color=default_color, marker=default_marker,
                       edgecolor='k', s=_point_size(df,size), alpha=0.7)
            else:
                ...
                for m in marker_vals:
                    ...
                    ax.scatter(group[x], group[y], color=default_color, marker=marker_map[m],
                           edgecolor='k', s=_point_size(group,size), alpha=0.7)

                # Marker legend
                marker_legend = [Line2D([0], [0], marker=marker_map[m], color='k', label=m,
                                    markerfacecolor='lightgray', markersize=8, linestyle='')
                                 for m in marker_vals]
        ...
        # Handle color as continuous
            ...
            if marker is None:
                scatter = ax.scatter(df[x], df[y], c=df[color], cmap='viridis', marker=default_marker,
                                 edgecolor='k', s=_point_size(df,size), alpha=0.7)
            else:
                marker_vals = df[marker].unique()
                marker_cycle = itertools.cycle(base_marker_styles)
                ...
                for m in marker_vals:
                    group = df[df[marker] == m]
                    ax.scatter(group[x], group[y], c=group[color], cmap='viridis', norm=norm,
                           marker=marker_map[m], edgecolor='k', s=_point_size(group,size), alpha=0.7)
            ...
            sm.set_array([])
            plt.colorbar(sm, ax=ax, label=color, orientation='vertical', pad=0.02, location='right')
        ...
        # Handle color as categorical
            ...
            if marker is None:
                for c in color_vals:
                    group = df[df[color] == c]
                    ax.scatter(group[x], group[y], color=color_map[c], marker=default_marker,
                           edgecolor='k', s=_point_size(group,size), alpha=0.7)
            else:
                ...
                for m in marker_vals:
                    for c in color_vals:
                        ...
                        ax.scatter(group[x], group[y], color=color_map[c], marker=marker_map[m],
                                   edgecolor='k', s=_point_size(group,size), alpha=0.7)

        # Custom legend for color (categorical)
        if color_is_categorical:
            color_legend = [Line2D([0], [0], marker='o', color='w', label=cat,
        ...
        if marker_is_categorical:
            marker_legend = [Line2D([0], [0], marker=marker_map[cat], color='k', label=cat,
                markerfacecolor='gray', markersize=8, linestyle='')
...
def _scatter_plotly(
    ...
    if marker:
        ...
        symbol_map = {m: marker_symbols[i % len(marker_symbols)] for i, m in enumerate(unique_markers)}
    ...
    fig.update_layout(
        ...
        yaxis_title=y,
        title=dict(
            text='Interactive ScatterGL Plot',
            x=0.5,
            xanchor='center'
        ),
...
class msa:
    ...
    def _rgb(self):
        ...
        self._rgb_array = np.zeros((self.df.shape[0], self.df.shape[1], 3), dtype=np.uint8)
    ...
    def logo(self,logotype='information', left=None, right=None):
        ...
        if self._count_df is None:
            ...
            seqs = self.df[self.df[self.pileup_col].astype(str).str.len() > 1][self.pileup_col].astype(str).tolist()
    ...
    def heatmap(self, xlabel='Position in sequence', ylabel='Row index', left=None, right=None, bottom=None, top=None, title=None):
        "Displays the RGB-based heatmap using matplotlib with optional axis labels and limits."
        if self._rgb_array is None: self._rgb()
    ...
    def _dssp_colors(self): return {
    'H': (0, 0, 255),     # blue
    'E': (255, 0, 0),     # red
    'L': (0, 255, 0),     # green
        }

    def _aa_colors(self): return {
    ...
    def _dssp_color_mapping(self): return {
    'H': 'blue',
    'E': 'red',
    'L': 'green'
        }
        
    def _aa_color_mapping(self): return {
    # Hydrophobic (green)
    'A': 'green',
    'V': 'green',
    'L': 'green',
...
def _dual_marker_plotly(
    ...
    colors='gray'
    if color:colors = df[color].unique()
    symbols = None
    if marker: symbols = df[marker].unique()

    # Define marker symbols (extend as needed)
    marker_symbols = ['circle', 'square', 'diamond', 'cross', 'x', 'triangle-up', 'triangle-down']
    symbol_map = {s: marker_symbols[i % len(marker_symbols)] for i, s in enumerate(symbols)}

    # Define colors using Plotly default palette
    color_palette = ['#636EFA', '#00CC96', '#FFA15A', '#19D3F3', '#FF6692', '#B6E880', '#FF97FF', '#FECB52']
...
def diff_df(df1, df2, CLAN, TRUE_df, title='two-set comparison', plotfun='scatter'):
  ...
  def evaluate_two(df1,df2):
    ...
    # Step 2: Outer join
    merged = pd.merge(df1_max, df2_max, on='sbjct', how='outer', suffixes=('_1', '_2'))
  ...
  result=evaluate_two(df1,df2).drop_duplicates()[['sbjct','z-score','ali-length','status']]
  if CLAN is not None:
