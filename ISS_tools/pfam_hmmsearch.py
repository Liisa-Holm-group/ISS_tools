...
def add_pfam_annotation(df, seq_col, pfamdir, verbose=True):
    ...
    try:
        if verbose:
            print(
                f"Running Pfam scan on sequences from column '{seq_col}'...",
                file=sys.stderr,
            )
...
def add_clan_annotation(df, clan_file):
    ...
    result = pd.merge(df, clan_df, left_on="pfam", right_on="AC", how="left")[
        ["sbjct", "pfam", "CL", "hmmer-evalue"]
    ]
...
def run_pfam_scan(fasta_file, hmm_file):
    ...
    with tempfile.NamedTemporaryFile(
        mode="w+", delete=False
    ) as tmpout1, tempfile.NamedTemporaryFile(mode="w+", delete=False) as tmpout2:
        try:
